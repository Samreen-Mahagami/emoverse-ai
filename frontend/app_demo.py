...
def generate_lesson_plan_pdf(plan):
    ...
    try:
        ...
        try:
            ...
            # Final fallback - create a simple text-based "PDF" (actually formatted text)
            lesson_text = f"""
═══════════════════════════════════════════════════════════════════════════════
...
✏️ PRACTICE ACTIVITY ({plan.get('phases', {}).get('practice_activity', {}).get('duration_minutes', 0)} minutes):
...
def display_lesson_plan(plan):
    ...
    try:
        ...
        # Always download as PDF format (even if it's formatted text)
        st.download_button(
        ...
        # Show info about PDF generation method
        if not pdf_data.startswith(b'%PDF'):
            st.info("💡 Downloaded as formatted PDF. For enhanced PDF features, additional libraries can be installed.")
    
    except Exception as e:
        st.error(f"❌ Error generating download: {str(e)}")
        # Provide basic PDF download as ultimate fallback
        lesson_text = f"""
═══════════════════════════════════════════════════════════════════════════════
                        📚 LESSON PLAN - {plan.get('lesson_title', 'Lesson Plan')}
═══════════════════════════════════════════════════════════════════════════════
...
"""
        st.download_button(
            ...
            file_name=f"lesson_plan_basic.pdf",
            mime="application/pdf",
            use_container_width=True
        )
