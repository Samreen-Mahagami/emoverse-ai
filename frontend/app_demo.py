...
def create_minimal_pdf(plan):
    """Create a simple HTML-based PDF using weasyprint or fallback to text"""
    try:
        # Try weasyprint for HTML to PDF conversion
        import weasyprint
        from io import BytesIO
        ...
        <head>
            ...
            <style>
                ...
                .phase-title {{ color: #2d3748; font-size: 14px; font-weight: bold; margin: 15px 0 8px 0; }}
        ...
        <body>
            <div class="header">
                <div class="title">📚 {plan.get('lesson_title', 'Lesson Plan')}</div>
            </div>
            
            <div class="info">
                <strong>Grade Level:</strong> Grade {plan.get('grade_level')}<br>
                <strong>Duration:</strong> {plan.get('duration_minutes')} minutes<br>
                <strong>Content Source:</strong> {plan.get('content_source', 'Based on uploaded material')}
            </div>
            
            <div class="section">
                <div class="section-title">🎯 Learning Objectives</div>
                <ul>
                    {''.join(f'<li>{obj}</li>' for obj in plan.get('learning_objectives', []))}
                </ul>
            </div>
            
            <div class="section">
                <div class="section-title">💡 SEL Competencies</div>
                <ul>
                    {''.join(f'<li>{comp}</li>' for comp in plan.get('sel_competencies', []))}
                </ul>
            </div>
        """
        ...
        # Add phases
        phase_info = [
            ('🔥 Warm-up', phases.get('warmup', {}), 'activities'),
            ('📖 Reading & Discussion', phases.get('reading_discussion', {}), 'activities'),
            ('✏️ Practice Activity', phases.get('practice_activity', {}), 'activities'),
            ('🎯 Wrap-up', phases.get('wrap_up', {}), 'methods')
        ]
        
        for title, phase_data, key in phase_info:
            duration = phase_data.get('duration_minutes', 0)
            items = phase_data.get(key, [])
            html_content += f"""
            <div class="section">
                <div class="phase-title">{title} ({duration} minutes)</div>
                <ul>
        ...
        # If weasyprint not available, return formatted text as fallback
        # But still return it as bytes so it can be downloaded
        lesson_text = f"""
...
def generate_lesson_plan_pdf(plan):
    """Generate a PDF for the lesson plan using fpdf2 (lightweight PDF library)"""
    try:
        # Try using fpdf2 first (more likely to be available)
        ...
        class LessonPlanPDF(FPDF):
            def header(self):
                ...
                self.cell(0, 10, 'EmoVerse AI - Lesson Plan', 0, 1, 'C')
                self.ln(10)
            ...
            def footer(self):
                ...
                self.cell(0, 10, 'Generated by EmoVerse AI - Social Emotional Learning Platform', 0, 0, 'C')
        ...
        pdf.cell(0, 8, f"Grade Level: Grade {plan.get('grade_level')}", 0, 1)
        pdf.cell(0, 8, f"Duration: {plan.get('duration_minutes')} minutes", 0, 1)
        pdf.cell(0, 8, f"Content Source: {plan.get('content_source', 'Based on uploaded material')}", 0, 1)
        pdf.ln(5)
        ...
        try:
            ...
            with PdfPages(buffer) as pdf_pages:
                ...
                # Title
                ax.text(0.5, 0.95, f"📚 {plan.get('lesson_title', 'Lesson Plan')}", 
                       fontsize=20, weight='bold', ha='center', color='#667eea')
                
                # Basic info
                y_pos = 0.88
                ax.text(0.1, y_pos, f"Grade Level: Grade {plan.get('grade_level')}", fontsize=12, weight='bold')
                y_pos -= 0.03
                ax.text(0.1, y_pos, f"Duration: {plan.get('duration_minutes')} minutes", fontsize=12, weight='bold')
                y_pos -= 0.05
                
                # Learning Objectives
                ax.text(0.1, y_pos, "🎯 Learning Objectives", fontsize=14, weight='bold', color='#667eea')
                y_pos -= 0.03
                for obj in plan.get('learning_objectives', []):
                    ax.text(0.12, y_pos, f"• {obj}", fontsize=10)
                    y_pos -= 0.025
                y_pos -= 0.02
                
                # SEL Competencies
                ax.text(0.1, y_pos, "💡 SEL Competencies", fontsize=14, weight='bold', color='#667eea')
                y_pos -= 0.03
                for comp in plan.get('sel_competencies', []):
                ...
                # Warm-up
                warmup = phases.get('warmup', {})
                ax.text(0.1, y_pos, f"🔥 Warm-up ({warmup.get('duration_minutes', 0)} minutes)", 
                       fontsize=12, weight='bold', color='#ff6b6b')
                y_pos -= 0.025
                for activity in warmup.get('activities', []):
                    ax.text(0.12, y_pos, f"• {activity}", fontsize=9)
                    y_pos -= 0.02
                y_pos -= 0.015
                ...
                # Footer
                ax.text(0.5, 0.05, "Generated by EmoVerse AI - Social Emotional Learning Platform", 
            ...
            # Final fallback - create a minimal valid PDF using basic PDF structure
            return create_minimal_pdf(plan)
...
def display_lesson_plan(plan):
    ...
    # Generate download for lesson plan
    try:
        pdf_data = generate_lesson_plan_pdf(plan)
        
        # Check if we got a proper PDF or text fallback
        if pdf_data.startswith(b'%PDF') or len(pdf_data) > 1000:  # Likely a real PDF
            st.download_button(
                label="📥 Download Lesson Plan (PDF)",
                data=pdf_data,
                file_name=f"lesson_plan_{plan.get('lesson_title', 'plan').replace(' ', '_').replace(':', '').replace('?', '').replace('!', '')}.pdf",
                mime="application/pdf",
                use_container_width=True
            )
        else:
            # Fallback to text download if PDF generation failed
            st.download_button(
            ...
            st.info("💡 PDF libraries not available. Downloaded as formatted text file.")
