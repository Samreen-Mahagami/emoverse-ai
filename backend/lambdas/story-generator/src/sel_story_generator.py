...
"""
        ...
        if job_id:
            try:
                ...
                table.update_item(
                    Key={'job_id': job_id},
                    UpdateExpression='SET story_status = :status, story_result = :result, story_s3_key = :s3key',
                    ExpressionAttributeValues={
                        ':status': 'completed',
                        ':result': {'title': story_title, 'theme': story_theme},
                        ':s3key': f's3://{bucket_name}/{s3_key}'
                    }
                )
